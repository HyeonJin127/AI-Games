...
Action = int
# ✅ State 타입 수정: 단순 (x,y)가 아닌 확장된 상태
State = Tuple[Tuple[int, int], int, bool, bool, bool]
...
def get_state_tensor(env):
    state = [
        env.ai1_pos[0] / 15, env.ai1_pos[1] / 11,
        (env.ai2_pos[0] - env.ai1_pos[0]) / 15,
        (env.ai2_pos[1] - env.ai1_pos[1]) / 11,
        env.items_ai1 / 3.0,
        1.0 if env.used_item_ai1 else 0.0,
        1.0 if env.checkpoint_cleared_ai1 else 0.0,
        1.0 if env.unlocked_ai1 else 0.0
    ]
    return torch.FloatTensor(state).unsqueeze(0)

# ========== ✅ 추가: train_qlearning.py와 동일한 상태 함수 ==========
def get_state_ai1(env: MazeEnv) -> State:
    """
    train_qlearning.py와 동일한 상태 정의
    (위치, 코인개수, 아이템적중여부, 체크포인트도달, 언락여부)
    """
    return (
        env.ai1_pos,
        env.items_ai1,
...
def load_q_table(table_name="q_table_maze_v4"):
    print(f"[INFO] Supabase 테이블 '{table_name}'에서 모든 Q-테이블 로드 중...")
    try:
        all_data = []
        offset = 0
        limit = 1000
        ...
        if not all_data:
            print(f"[WARN] '{table_name}' 테이블에 데이터가 없습니다.")
            return None
        ...
        print(f"[SUCCESS] Q-테이블 로드 완료: 총 {success_count}개의 데이터를 불러왔습니다.")
        return Q
        ...
        if game_state == STATE_GAME:
            if not paused:
                ...
                if selected_mode == MODE_AI_VS_AI:
                    ...
                    # ✅ AI1 - 수정된 부분: 5차원 상태 사용
                    if USE_TRAINED_AI1 and q_loaded and Q is not None:
                        state_ai1 = get_state_ai1(env)  # ← 수정됨!
